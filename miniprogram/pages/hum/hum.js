...
/*在进入该页面时
  ...
  -> checkhour 将作为 hour 参数传入that.get_hum_info(hour)函数,获得日历选中日期的checkhour小时数据，并处理得到折线图数据：平均数数组： data1  最大数数组：data2
  -> 在1.2秒后执行更新数据函数updateData()，该函数会取this.data.data1 和 this.data.data2 进行遍历并更新到折线图中
*/
...
getdefaultinfo(){
  ...
  try {
    ...
    console.log("获取失败")
      ...
      console.log("在缓存中找到 hum安全值设置")
        that.setData({
          Stepper2:Stepper2 == null ? 300 : Stepper2,
        })
  }
},
...
get_hum_info(hour){
  ...
  //console.log(start)
  wx.request({
    url: `https://api.heclouds.com/devices/${url}/datapoints?datastream_id=hum&start=${start}&end=${end}&limit=230&sort=DESC`,
    header:{
      "api-key":`${apikey}`
    },
    method:"GET",
    success:function(e){
      ...
      const min = Math.min(...childArr);
      //console.log(max)
      data1.push(average.toFixed(1))
      data2.push(max)
      data3.push(min)
      }else{
        var d0 = Math.random()*25
...
getmax: function () {
  var that = this 
  var max = Math.max(...that.data.data2)
  var min = Math.min(...that.data.data3)
  that.setData({
    d0 : [max,min]
  })
},
