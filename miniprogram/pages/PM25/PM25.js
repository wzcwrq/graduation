...
/*在进入该页面时
  ...
  -> checkhour 将作为 hour 参数传入that.get_PM25_info(hour)函数,获得日历选中日期的checkhour小时数据，并处理得到折线图数据：平均数数组： data1  最大数数组：data2
  -> 在1.2秒后执行更新数据函数updateData()，该函数会取this.data.data1 和 this.data.data2 进行遍历并更新到折线图中
*/
...
get_PM25_info(hour){
  ...
  //console.log(start)
  wx.request({
    url: `https://api.heclouds.com/devices/${url}/datapoints?datastream_id=PM25&start=${start}&end=${end}&limit=230&sort=DESC`,
    header:{
      "api-key":`${apikey}`
    },
    method:"GET",
    success:function(e){
      ...
      data2.push(max)
        ...
        var d0 = Math.random()*9
        var dt = Math.random()*11
        var d1 = d0 + 25
        var d2 = d0 + 37 - dt
