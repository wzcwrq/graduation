...
getdefaultinfo(){
  ...
  try {
    ...
    console.log("获取失败")
      ...
      console.log("在缓存中找到 temp安全值设置")
        that.setData({
          Stepper1:Stepper1 == null ? 100 : Stepper1,
        })
  }
},
...
get_hum_info(hour){
  ...
  console.log(end)
  wx.request({
    url: `https://api.heclouds.com/devices/${url}/datapoints?datastream_id=temp&start=${start}&end=${end}&limit=230&sort=DESC`,
    header:{
      "api-key":`${apikey}`
    },
    method:"GET",
...
getmax: function () {
  var that = this 
  const data2 = that.data.data2;
  const data3 = that.data.data3;
  var max = Math.max(...data2)
  var min = Math.min(...data3)
  that.setData({
    d0 : [max,min]
  })
},
